...
| POST | `/v1/compliance-check-monitoring-state/initiate` | Initiate |
| GET | `/v1/compliance-check-monitoring-state` | Retrieve (list) |
| GET | `/v1/compliance-check-monitoring-state/{crId}/retrieve` | Retrieve |
| PUT | `/v1/compliance-check-monitoring-state/{crId}/update` | Update |
| PUT | `/v1/compliance-check-monitoring-state/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/compliance-check-monitoring-state/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-guideline-compliance:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-guideline-compliance ./helm -n bian-risk-compliance
```

Exposed through the platform Gateway at path prefix `/sd-guideline-compliance` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
